<script lang="ts">
	...
	const COLOR_FIELDS = [
		...
		{ key: 'accentForeground' as const, label: 'Accent Foreground', description: 'Text on accents' },
		{ key: 'destructive' as const, label: 'Destructive', description: 'Danger actions' },
		{ key: 'destructiveForeground' as const, label: 'Destructive Foreground', description: 'Text on danger' },
	...
	async function loadCustomThemes() {
		...
		try {
			const allThemes = await orpc.theme.getAll();
			...
			const themes: CustomTheme[] = allThemes
				...
				.map((theme) => ({
					...
					isActive:
						...
						(!currentThemeId && currentTheme?.id === theme.id),
	...
	async function handleUpdateTheme() {
		...
		try {
			...
			const themeIndex = customThemes.findIndex((t) => t.id === editingTheme.id);
			if (themeIndex !== -1) {
			...
			setTimeout(() => {
				...
				if (onThemeUpdated) {
					onThemeUpdated(editingTheme.id);
				}
			}, 1500);
		} catch (error) {
	...
	async function handleDeleteTheme() {
		...
		try {
			...
			customThemes = customThemes.filter((t) => t.id !== deletingTheme.id);
			...
			// Notify parent component
			if (onThemeDeleted) {
				onThemeDeleted(deletingTheme.id);
			}
		} catch (error) {
...
<Card class={cn('overflow-hidden', className)}>
	...
	<CardContent class="space-y-4">
		...
		{:else if error}
			<!-- Error state -->
			<div class="border-destructive/50 bg-destructive/10 flex items-center gap-2 rounded-lg border p-4">
						...
						{#if theme.isActive}
							<div class="border-primary bg-primary/10 border-b px-2 py-1">
								<div class="flex items-center gap-1.5">
									...
									<span class="text-primary text-xs font-semibold uppercase">
										Active Theme
									</span>
								</div>
							</div>
...
<Dialog.Root bind:open={editDialogOpen}>
	...
	>
		<Dialog.Header>
			...
			<Dialog.Description>
				Update your theme's name, description, or colors. Changes will be applied immediately
				if this is your active theme.
			</Dialog.Description>
		</Dialog.Header>
...
<Dialog.Root bind:open={deleteDialogOpen}>
	...
	>
		<Dialog.Header>
			...
			<Dialog.Description>
				This action cannot be undone. The theme will be permanently removed from your
				collection.
			</Dialog.Description>
		</Dialog.Header>
		...
		<div class="space-y-4 py-4">
			{#if deletingTheme}
				...
				{#if deletingTheme.isActive}
					<div class="border-primary/50 bg-primary/10 text-primary flex items-start gap-2 rounded-md border p-3 text-sm">
						...
						<div>
							...
							<p class="text-primary/80 mt-1 text-xs">
								You are about to delete your active theme. Your appearance will be reset to
								the Light theme after deletion.
							</p>
						</div>
					</div>
				{/if}
			...
			{#if deleteErrorMessage}
				<div class="bg-destructive/10 text-destructive border-destructive/50 flex items-start gap-2 rounded-md border p-3 text-sm">
					<AlertTriangleIcon class="size-4 shrink-0 mt-0.5" />
					<div>
