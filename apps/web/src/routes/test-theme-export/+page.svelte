			...
			<Card>
				...
				<CardContent class="space-y-4">
					...
					<div class="border-border border-t pt-4">
						...
						<code class="text-muted-foreground block rounded bg-muted p-2 text-xs">
							theme_&lt;name&gt;_&lt;date&gt;.json
						</code>
						<p class="text-muted-foreground mt-2 text-xs">
							Example: theme_dark_2025-01-26.json
						</p>
					</div>

					<div class="border-border border-t pt-4">
						<h3 class="mb-2 font-semibold">Export Schema:</h3>
						<pre class="text-muted-foreground overflow-x-auto rounded bg-muted p-2 text-xs"
>{`{
  "name": string,
  "description"?: string,
  "colors": ThemeColors,
			...
			<Card>
				<CardHeader>
					<div class="flex items-center justify-between">
						...
						<span class="border-border border rounded px-2 py-1 text-xs">Interactive</span>
					</div>
					<CardDescription>
					...
					<div class="space-y-4">
						{#each themes as theme, index}
							<div>
								...
								<pre
									class="text-muted-foreground max-h-64 overflow-auto rounded-lg bg-muted p-4 text-xs"
								><code>{previewExport(theme)}</code></pre
								>
							</div>
						{/each}
					</div>
							...
							<div class="border-border flex items-start justify-between rounded-lg border p-4">
								<div class="flex-1">
									<div class="mb-2 flex items-center gap-2">
										<h4 class="font-semibold">{theme.name}</h4>
										{#if validation.valid}
											<span class="bg-primary text-primary-foreground flex items-center gap-1 rounded px-2 py-1 text-xs">
												<Check class="size-3" />
												Valid
											</span>
										{:else}
											<span class="bg-destructive text-destructive-foreground flex items-center gap-1 rounded px-2 py-1 text-xs">
									...
									{#if !validation.valid && validation.errors.length > 0}
										<ul class="text-destructive list-disc list-inside text-sm">
