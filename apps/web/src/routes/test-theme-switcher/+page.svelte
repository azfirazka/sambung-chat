<script lang="ts">
	...
	function resetToDefault() {
		appearance.updateSetting('themeId', null);
		themeChangeCount++;
		lastThemeChange = new Date().toLocaleTimeString();
			...
			<Card>
				...
				<CardContent class="space-y-4">
					<div class="space-y-2">
						...
						<ol class="text-muted-foreground list-decimal list-inside space-y-1 text-sm">
		...
		<div class="space-y-6 lg:col-span-2">
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Select a theme to apply it to the entire application
					</CardDescription>
				</CardHeader>
				<CardContent>
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Real-time display of current theme and settings
					</CardDescription>
				</CardHeader>
				<CardContent class="space-y-4">
					<div class="grid gap-4 md:grid-cols-2">
						...
						<div class="border-border bg-muted/30 rounded-lg border p-3">
							...
							<p class="text-foreground text-sm font-mono break-all">
						...
						<div class="border-border bg-muted/30 rounded-lg border p-3">
							...
							<div class="space-y-1 text-sm">
								<p class="text-foreground">
									<strong>Name:</strong> {currentThemeInfo.name}
								</p>
								<p class="text-foreground">
									<strong>ID:</strong> {currentThemeInfo.id}
								</p>
								<p class="text-foreground">
									<strong>Type:</strong>
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Mark each item as you verify it works correctly
					</CardDescription>
				</CardHeader>
				<CardContent>
					<div class="space-y-2">
						{#each [
							'Theme switcher component renders without errors',
							'All built-in themes (Light, Dark, High Contrast) are displayed',
							'Each theme shows name, description, and color preview swatches',
							'Clicking a theme applies it visually to the entire page',
							'Selected theme shows checkmark and highlighted border',
							'Color preview swatches accurately represent theme colors',
							'"Current Selection" summary shows correct theme',
							'Theme change counter increments when selecting themes',
							'Theme changes persist after page reload',
							'Reset to Default button clears the theme selection',
							'UI component preview shows correct theme colors',
							'Loading state displays while themes are being fetched',
							'Error handling works if backend is unavailable',
							'Custom themes (if any) are loaded and displayed',
