<script lang="ts">
	...
	import { importThemeFromFile, importMultipleThemes, validateThemeImport, createFileInput } from '$lib/utils/theme-import';
...
<div class="container mx-auto max-w-6xl p-6 space-y-6">
			...
			<div class="card p-4 space-y-3">
				...
				<div class="space-y-2">
					...
					<button
						...
						onclick={testWithCustomJSON}
					>
						<FileJsonIcon class="w-4 h-4 mr-2" />
						Test Custom Theme
					</button>
					<button
						class="w-full btn btn-destructive text-sm py-2"
						onclick={testWithInvalidJSON}
					>
			...
			<div class="card p-6">
				...
				>
					...
					<p class="text-lg font-medium mb-2">
						Drag & drop theme JSON file here
					</p>
					...
					<div class="space-y-2">
						...
						<button class="w-full btn btn-outline" onclick={openFileDialog}>
							Open File Picker
						</button>
					</div>
				...
				<!-- Validation Info -->
				<div class="mt-4 p-3 bg-muted rounded text-sm space-y-1">
				...
				{:else}
					<div class="space-y-3 max-h-96 overflow-y-auto">
						{#each testResults as result}
							<div
								class="p-3 rounded border {result.success
							...
							>
								<div class="flex items-start justify-between mb-2">
									...
									<span
										class="text-xs px-2 py-1 rounded {getStatusColor(result.success)}"
									>
								...
								{#if result.success && result.theme}
									<div class="text-xs mt-2 p-2 bg-black/5 dark:bg-white/5 rounded">
										<div class="font-medium mb-1">Imported Theme Data:</div>
							...
							<div class="flex items-center justify-between py-2 border-b last:border-0">
								<div class="flex items-center gap-2">
									...
									{:else}
										<AlertCircleIcon class="w-4 h-4 text-red-600" />
									{/if}
									<span class="truncate max-w-[150px]" title={item.filename}>
										{item.filename}
									</span>
								</div>
	...
	<!-- API Reference -->
	<div class="card p-6">
		<h2 class="font-semibold text-lg mb-4">API Reference</h2>
		<div class="grid md:grid-cols-2 gap-6">
			<div>
				<h3 class="font-medium mb-2">Main Functions</h3>
				<ul class="space-y-2 text-sm">
					<li>
						<code class="text-xs bg-muted px-2 py-1 rounded"
							>importThemeFromFile(file: File)</code
						>
						<br />
					...
					<li>
						<code class="text-xs bg-muted px-2 py-1 rounded">createFileInput()</code>
						<br />
			...
			<div>
				<h3 class="font-medium mb-2">Types</h3>
				<ul class="space-y-2 text-sm">
					<li>
						<code class="text-xs bg-muted px-2 py-1 rounded">ThemeImportResult</code>
						<br />
					...
					<li>
						<code class="text-xs bg-muted px-2 py-1 rounded">CreateThemeData</code>
						<br />
						<span class="text-muted-foreground"
	...
	<div class="card p-6">
		<h2 class="font-semibold text-lg mb-4">Manual Verification Checklist</h2>
		<div class="grid md:grid-cols-2 gap-6">
			<div>
				<h3 class="font-medium mb-3">Valid Imports</h3>
				<ul class="space-y-2 text-sm">
			...
			<div>
				<h3 class="font-medium mb-3">Error Handling</h3>
				<ul class="space-y-2 text-sm">
					<li class="flex items-center gap-2">
						<input type="checkbox" class="w-4 h-4" />
