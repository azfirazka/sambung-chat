...
export function getCSPHeader(config: CSPConfig = { reportOnly: false }): string {
	...
	const publicApiUrl =
		typeof process !== 'undefined' ? (process.env.PUBLIC_API_URL || 'http://localhost:3000') : 'http://localhost:3000';

	// Get Keycloak URL if configured
	const keycloakUrl = typeof process !== 'undefined' && process.env.KEYCLOAK_URL
...
export function getSecurityHeaders(config: SecurityHeadersConfig = {}): Record<string, string> {
	...
	const {
		cspReportOnly = false,
		cspReportUri,
		includeHSTS = nodeEnv === 'production',
	} = config;
