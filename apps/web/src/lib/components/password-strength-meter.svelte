...
{#if shouldRender}
  <div class={cn('space-y-2', className)} aria-live="polite" aria-atomic="true" {...restProps}>
    ...
    {#if showSuggestions && strength.suggestions.length > 0}
      <div class="bg-muted/50 rounded-md p-3">
        <p class="text-muted-foreground mb-1.5 text-sm font-medium">Suggestions to improve:</p>
        <ul class="space-y-1">
