<script lang="ts">
	...
	async function loadThemes() {
		...
		try {
			...
			try {
				const customThemes = await orpc.theme.getAll();
				...
				for (const customTheme of customThemes) {
					...
					const theme: Theme = {
						...
						colors: customTheme.colors as Theme['colors'],
				...
				<div class="grid gap-3">
					{#each availableThemes as option}
						...
						<button
							...
							disabled={isApplying}
							class="border-input hover:border-primary hover:bg-accent/50 data-[active=true]:border-primary data-[active=true]:bg-primary/5 data-[active=true]:ring-1 data-[active=true]:ring-primary disabled:opacity-50 disabled:cursor-not-allowed relative flex cursor-pointer items-start gap-3 rounded-lg border p-4 text-left transition-all"
						...
						>
							...
							<div class="flex-1 space-y-2">
								<div class="flex items-center justify-between">
									...
									{#if isApplying}
										<span
											class="text-muted-foreground text-xs animate-pulse font-medium uppercase"
										>
											Applying...
										</span>
									{/if}
