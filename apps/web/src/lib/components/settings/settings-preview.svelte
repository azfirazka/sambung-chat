<script lang="ts">
   ...
   */
  const DENSITY_SPACING: Record<MessageDensity, { messagePadding: string; messageGap: string; avatarSize: string; textSize: string }> = {
    compact: {
  ...
  const sampleMessages = [
    ...
    },
    {
      ...
      content: 'Of course! Message density controls the spacing and layout of messages in your conversations. You can choose between compact, comfortable, and spacious options to suit your reading preference.'
    },
    {
      role: 'user' as const,
    ...
    {
      role: 'assistant' as const,
      content: 'Font size adjusts the text size from 12px to 20px, while font family lets you choose between system-ui, sans-serif, or monospace fonts. These settings help personalize your reading experience.'
    }
  ];
...
<Card class={cn('overflow-hidden', className)}>
  ...
  <CardContent>
    ...
    <div class="border-border bg-muted/30 rounded-lg border p-4">
      <div
        class="space-y-0"
        style="gap: {currentSpacing.messageGap};"
      >
      ...
      <!-- Settings Summary -->
      <div class="border-border mt-4 flex flex-wrap items-center justify-between gap-2 border-t pt-3">
        <div class="flex flex-wrap gap-x-4 gap-y-1 text-xs">
          ...
          <div class="flex items-center gap-1">
            ...
            <span class="text-foreground font-medium">{appearance.currentSettings.messageDensity}</span>
          </div>
        </div>
      </div>
    </div>
    ...
    {#if browser}
      <div class="mt-3 rounded-md bg-muted/50 p-3">
        <p class="text-muted-foreground text-xs leading-relaxed">
          <span class="font-semibold">Tip:</span> Adjust the settings above to see real-time changes
          in this preview. The preview shows how messages will appear in your conversations.
        </p>
      </div>
    {/if}
  </CardContent>
