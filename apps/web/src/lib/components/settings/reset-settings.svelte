      ...
      <div class="text-sm">
        {#if hasCustomSettings()}
          ...
          <p class="text-muted-foreground text-xs">
            {settingsToReset().length} setting{settingsToReset().length !== 1 ? 's' : ''} will be reset to defaults
          </p>
        {:else}
        ...
        <div class="space-y-1">
          {#each settingsToReset() as setting}
            <div class="flex items-center justify-between text-xs">
              ...
              <div class="flex items-center gap-2">
                <span class="text-muted-foreground line-through decoration-destructive">
                  {setting.current}
                </span>
...
<Dialog.Root bind:open={dialogOpen}>
  ...
  >
    <Dialog.Header>
      ...
      <Dialog.Description>
        This action cannot be undone. All appearance settings will be restored to their
        default values.
      </Dialog.Description>
    </Dialog.Header>
    ...
    <div class="space-y-4 py-4">
      ...
      {#if resetSuccess}
        <div class="bg-primary/10 text-primary flex items-start gap-2 rounded-md p-3 text-sm">
          <RotateCcwIcon class="size-4 shrink-0 mt-0.5" />
          <div>
