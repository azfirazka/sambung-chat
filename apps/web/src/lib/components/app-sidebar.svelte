<script lang="ts" module>
  ...
  // Navigation data (user will be passed as prop)
  const navData = {
    navMain: [
      {
        title: 'Inbox',
...
<script lang="ts">
  ...
  let activeItem = $state(navData.navMain[0]);
  let mails = $state(navData.mails);
            ...
            {#each navData.navMain as item (item.title)}
              <Sidebar.MenuItem>
                <Sidebar.MenuButton
                  ...
                  onclick={() => {
                    activeItem = item;
                    const mail = navData.mails.sort(() => Math.random() - 0.5);
