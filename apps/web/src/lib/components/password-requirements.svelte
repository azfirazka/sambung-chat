<script lang="ts">
  ...
  let { password, class: className, ...restProps }: PasswordRequirementsProps = $props();
    ...
    {#each requirements as requirement}
      <div
        ...
        aria-label={requirement.met
          ...
          : `${requirement.label} - not met`}
      >
        {#if requirement.met}
          <CheckCircle2Icon class="h-4 w-4 flex-shrink-0 text-green-500" aria-hidden="true" />
        {:else}
          <XCircleIcon class="text-muted-foreground h-4 w-4 flex-shrink-0" aria-hidden="true" />
        {/if}
        <span
          class={cn(
