 ...
 *
 * Handles Mermaid diagram rendering with lazy-loading
 * Provides theme detection, initialization, and rendering of Mermaid diagrams
 */

import { loadMermaid } from '$lib/utils/lazy-load';
...
/**
 * Render Mermaid code block to HTML
 */
export function renderMermaidCode(code: string, escaped?: boolean): string {
  const diagramId = `mermaid-diagram-${mermaidCounter++}`;

  // First escape to sanitize any potential XSS
  let mermaidCode = escaped ? code : escapeHtml(code);
...
/**
 * Detect if text contains Mermaid diagrams
 */
export function detectMermaid(text: string): boolean {
  return /```mermaid\b/.test(text) || /`{3}mermaid\b/.test(text);
}
...
/**
 * Check if Mermaid is currently loaded
 */
export function isLoaded(): boolean {
  return typeof window !== 'undefined' && (window as any).mermaid !== undefined;
}
