...
describe('ApiKeyService', () => {
	...
	describe('validateKey', () => {
		...
		it('should throw for null input', () => {
			expect(() => ApiKeyService.validateKey(null as any)).toThrow('API key must be a non-empty string');
		});

		it('should throw for undefined input', () => {
			expect(() => ApiKeyService.validateKey(undefined as any)).toThrow('API key must be a non-empty string');
		});

		it('should throw for non-string input', () => {
			expect(() => ApiKeyService.validateKey(123 as any)).toThrow('API key must be a non-empty string');
		});

		it('should throw for keys shorter than 8 characters', () => {
			const shortKey = 'sk-12';

			expect(() => ApiKeyService.validateKey(shortKey)).toThrow('API key appears to be invalid (too short)');
		});

		it('should throw for keys longer than 500 characters', () => {
			const longKey = 'sk-' + 'a'.repeat(500);

			expect(() => ApiKeyService.validateKey(longKey)).toThrow('API key appears to be invalid (too long)');
		});
	...
	describe('encryptAndStore', () => {
		...
		it('should throw INTERNAL_ERROR when encryption fails', async () => {
			...
			await expect(ApiKeyService.encryptAndStore(input)).rejects.toThrow('Failed to encrypt API key');
	...
	describe('retrieveAndDecrypt', () => {
		...
		it('should throw INTERNAL_ERROR when decryption fails', async () => {
			...
			await expect(ApiKeyService.retrieveAndDecrypt(id, userId)).rejects.toThrow('Failed to decrypt API key');
			await expect(ApiKeyService.retrieveAndDecrypt(id, userId)).rejects.toThrow(ORPCError);
		});
	});
	...
	describe('updateKey', () => {
		...
		it('should update key with encryption', async () => {
			...
			// Mock update
			const updatedKey = { ...existingKey, encryptedKey: mockEncryptedData.encrypted, keyLast4: '6789' };
			const mockWhereUpdate = vi.fn().mockReturnValue({
				returning: vi.fn().mockResolvedValue([updatedKey]),
			});
			mockSet.mockReturnValue({ where: mockWhereUpdate });
	...
	describe('User Isolation', () => {
		...
		it('should enforce user isolation in retrieveAndDecrypt', async () => {
			...
			await expect(ApiKeyService.retrieveAndDecrypt(id, differentUserId)).rejects.toThrow(ORPCError);
		});
