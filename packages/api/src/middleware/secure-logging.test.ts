...
describe('Secure Logging Middleware', () => {
	...
	describe('sanitizeErrorMessage', () => {
		describe('API key patterns', () => {
			...
			it('should redact JWT tokens (eyJ prefix)', () => {
				const message = 'Token error: eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIn0.signature';
				const result = sanitizeErrorMessage(message);
		...
		describe('Multiple occurrences', () => {
			it('should redact multiple API keys in same message', () => {
				const message = 'Keys sk-11111111111111111111111111111111 and sk-22222222222222222222222222222222 both failed';
				const result = sanitizeErrorMessage(message);
			...
			it('should redact mixed API key types', () => {
				const message = 'OpenAI key sk-11111111111111111111111111111111 and Google key AIzaSyDaGmWKa4JsXZ-HjGw7ISLn_3namBGewQe both failed';
				const result = sanitizeErrorMessage(message);
		...
		describe('Mixed safe and sensitive content', () => {
			it('should preserve safe content', () => {
				const message = 'Error: Failed to authenticate user john@example.com with key sk-1234567890abcdef1234567890abcdef';
				const result = sanitizeErrorMessage(message);

				expect(result).toBe('Error: Failed to authenticate user john@example.com with key [REDACTED]');
			});
	...
	describe('secureLoggingMiddleware', () => {
		...
		it('should sanitize generic Error message', async () => {
			const next = vi.fn().mockRejectedValue(
				new Error('Failed with key sk-1234567890abcdef1234567890abcdef')
			);
	...
	describe('String API key pattern detection', () => {
		describe('redactSensitiveData with API key patterns in strings', () => {
			...
			it('should redact multiple API keys in same string', () => {
				const input = { message: 'Keys sk-11111111111111111111111111111111 and sk-22222222222222222222222222222222 both work' };
				const result = redactSensitiveData(input);
	...
	describe('Real-world scenarios', () => {
		...
		it('should handle database query logging', () => {
			const query = {
				...
				params: ['user-123'],
				rows: [
					{ id: '1', encrypted_key: 'encrypted-data-123', provider: 'openai' },
				],
			};
