...
} from './apps/web/src/lib/utils/password-strength.ts';
...
testCases.forEach((testCase, index) => {
  ...
  console.log(
    `Password: "${testCase.password.substring(0, 20)}${testCase.password.length > 20 ? '...' : ''}"`
  );
  ...
  const scoreInRange =
    result.score >= testCase.expectedMinScore && result.score <= testCase.expectedMaxScore;
  ...
  if (success) {
    ...
    if (!scoreInRange) {
      console.log(
        `     Expected score range: ${testCase.expectedMinScore}-${testCase.expectedMaxScore}, got: ${result.score}`
      );
    }
    failed++;
  }
